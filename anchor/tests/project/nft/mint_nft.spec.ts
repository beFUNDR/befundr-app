...
describe("mint_nft", () => {
  ...
  beforeEach(async () => {
    ...
    const globalsPda = await updateAdmins(adminWallet, adminWallet, program, [
  ...
  describe("success cases", () => {
    it("successfully start the nft mint round of a project", async () => {
      ...
      const nftAllocationPda = await mintNft(
        userWallet,
        payerWallet,
        projectPda,
        MINT_ADDRESS,
      ...
      const nftAllocation =
        await program.account.nftAllocation.fetch(nftAllocationPda);
