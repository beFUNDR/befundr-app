...
import { getOrCreateATA, USDC_MINT_ADDRESS } from "@/shared/utils/ata-utils";
...
export const mintNft = async ({
  ...
  const [nftAllocationPda] = PublicKey.findProgramAddressSync(
    [
      Buffer.from("nft_allocation"),
      new PublicKey(project.projectPda).toBuffer(),
  ...
  const startNftMintRoundProjectTx = await program.methods
    ...
    .accountsPartial({
      ...
      payer: payer,
      from: (
        await getOrCreateATA(payer, authority, connection, sendTransaction)
      ).associatedToken,
      to: (await getOrCreateATA(payer, configPda, connection, sendTransaction))
        .associatedToken,
